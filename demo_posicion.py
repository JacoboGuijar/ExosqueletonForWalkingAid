...
def MoveToPositionSpeed(target_position,target_speed):
    while True:
        if target_speed != 0:
            result = epos.VCS_SetPositionProfile(keyHandle, nodeID, target_speed, acceleration, deceleration, byref(pErrorCode)) # set profile parameters

            result = epos.VCS_MoveToPosition(keyHandle, nodeID, target_position, True, True, byref(pErrorCode)) # move to position
...
if __name__ == "__main__":
    ...
    # print("resultado: " + str(ret)) #Success
    ...
    MoveToPositionSpeed(40000,5000) # move to position 20,000 steps at 5000 rpm/s
